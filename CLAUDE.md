...
- **Response summarization (opt-in)**: Set `CLAUDE_RESPONSE_SUMMARY_ENABLED=true` in `~/.env`
  ...
  - Summarizes in first person ("I added...") using LLM priority: **Anthropic** → OpenAI → Groq → simple fallback
  - Speaks summary via OpenAI nova voice (OpenAI → ElevenLabs → system voice)
